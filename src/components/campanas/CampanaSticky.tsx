...
/** Botón WhatsApp sticky con el mensaje propio de la landing (mismo patrón que StickyWhatsApp). */
export function CampanaSticky({ mensaje }: { mensaje: string }) {
  const [shown, setShown] = useState(false);

  useEffect(() => {
    const onScroll = () => setShown((window.scrollY || window.pageYOffset || 0) > 420);
